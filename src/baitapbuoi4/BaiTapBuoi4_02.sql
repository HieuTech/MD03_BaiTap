...
#1
select DMSV.MaSV, DMSV.TenSV, DMSV.HoSV, DMSV.NgaySinh, DMSV.Phai
...
select DMSV.TenSV, DMSV.NgaySinh, DMSV.HocBong
from DMSV
where YEAR(CURRENT_DATE) - YEAR(DMSV.NgaySinh) > 20;
...
select count(*) as TongSinhVienNu
from dmsv
where Phai = 'Nữ';

Select
            (select count(*) from dmsv) as TongSinhVien,
            (select count(*)
             from dmsv
             where Phai = 'Nữ') as TongSinhVienNu;
...
explain analyze select  (YEAR(CURRENT_DATE) - YEAR(DMSV.NgaySinh)) as DoTuoi , count(*) as TongSoSinhVien
from dmsv
group by  DoTuoi;

#9
select DK.TenKhoa ,max(DMSV.HocBong)
from dmsv
